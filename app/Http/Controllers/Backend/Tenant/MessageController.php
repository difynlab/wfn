...
class MessageController extends Controller
{
    ...
    public function index(Request $request, $category)
    {
        ...
        $starred_count = $user->messages()->where('status', 1)->where('favorite', 1)->get()->count();
        ...
        elseif($category == 'starred') {
            $items = $user->messages()->where('status', 1)->where('favorite', 1)->orderBy('id', 'desc')->paginate($pagination);
        }
        elseif($category == 'bin') {
        ...
        return view('backend.tenant.messages.index', [
            'items' => $items,
    ...
    public function create()
    {
        ...
        return view('backend.tenant.messages.create', [
            'all_count' => $all_count,
            'starred_count' => $starred_count,
            'bin_count' => $bin_count
        ]);
    ...
    public function store(Request $request)
    {
        ...
        if($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput()->with([
                ...
                'route' => route('tenant.messages.index', 'all')
            ]);
        }
        ...
        return redirect()->route('tenant.messages.index', 'all')->with([
    ...
    public function edit(Message $message)
    {
        ...
        return view('backend.tenant.messages.edit', [
            'message' => $message,
    ...
    public function filter(Request $request, $category)
    {
        ...
        elseif($category == 'starred') {
            $items = $user->messages()->where('status', 1)->where('favorite', 1)->orderBy('id', 'desc');
        }
        elseif($category == 'bin') {
